<template>
  <div
    ...
    ref="base"
  >
    <dl class="ct-dept-select__name" @click="showList" v-show="!visible">
      <span>{{ name }}</span>
    </dl>
    ...
    <fa-font class="ct-dept-select__arrow" name="angle-down" />
...
export default {
  ...
  computed: {
    ...
    listStyle() {
      ...
      // 保证有一定的高度
      let length =
        this.deptList.length < this.maxItem ?
  ...
  methods: {
    getList(payload) {
      ...
      const formdata = {
        ...
        pageindex: 1,
        allowpageing: 0,
        nolimitrecord: this.nolimitrecord,
        permissionenable: this.permissionenable,
        sys: this.sys,
        querymodel: options,
      }

      this.r().post('/ctoa/plat/OaSelectForm', formdata).then(response => {
    ...
    handleClick(payload) {
      ...
      if (this._.isArray(payload.label)) {
        if (this.level === '') {
          ...
          let arr = payload.label
          for (let i = arr.length; i--;) {
            if (arr[i] === '') {
              arr = arr.slice(0, arr.length - 1)
            }
          }
          this.label = arr.reverse().slice(0, this.level).toString().replace(/,/g, this.dot)
